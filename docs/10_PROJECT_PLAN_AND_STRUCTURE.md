...
> **WEEK 2 완료 (2026-01-21)**
>
> ## 주간 목표 달성
> "규정 위반 감지 + 장애 원인 분석 + 업무 실행 계획" — LLM 기반 판단 및 분석 파이프라인 구축 완료.
>
> ## 완료된 작업
>
> | 작업 | 내용 | 상태 |
> |------|------|------|
> | W2-1 | Knowledge Store Upload Pipeline | ✓ 완료 |
> | W2-2 | 규정 위반 감지 서브그래프 | ✓ 완료 |
> | W2-3 | 장애 RCA 서브그래프 | ✓ 완료 |
> | W2-4 | 업무 실행 계획 서브그래프 | ✓ 완료 |
> | W2-5 | 서브그래프 통합 | ✓ 완료 |
>
> ## 주요 구현 사항
>
> 1. **Knowledge Store**: PDF 파싱 → 텍스트 청킹 → 임베딩 생성 → FAISS 벡터 저장
> 2. **Compliance 서브그래프**: 정책 검색 → LLM 규정 분석 → 위반 감지 및 권고 생성
> 3. **RCA 서브그래프**: 로그 파싱 → 유사 장애 검색 → 가설 생성 → 우선순위 정렬
> 4. **Workflow 서브그래프**: 요청 분석 → Action Plan 생성 → 위험도 평가 → 승인 필요 여부 결정
> 5. **서브그래프 통합**: mixed intent 분류 → 다중 서브그래프 순차 실행 → 결과 병합 및 통합 요약
